...
uint16_t 		ADC_GetValue	[1024];
extern uint16_t ADC_EndFlag;
...
void ADC_Get(void)
{
	...
	while(ADC_EndFlag == 1){delay_ms(5);}//等待ADC采集完成
}
...
{  
	...
	if(draw_mode==0)
	{
		...
		for(i=0;i<380;i++)
		{
			posX1 = i;
			posX2 = posX1 + 1;
			posY1 = 270 - (ADC_GetValue[i] * (270.0f / 4096.0f));
			posY2 = 270 - (ADC_GetValue[i+1] * (270.0f / 4096.0f));
	...
	if(draw_mode==1)
	{
		...
		for(i=1;i<380;i++)
		{
			posX1 = i;
			posX2 = posX1 + 1;
			posY1 = 90 - (fft_outputbuf[i] * (400.0f / 4096.0f));
			posY2 = 90 - (fft_outputbuf[i+1] * (400.0f / 4096.0f));
			printf("line %d,%d,%d,%d,%d\xff\xff\xff", posX1, posY1, posX2, posY2, 31);
		}
		for(i=0;i<380;i++)
		{
			posX1 = i;
			posX2 = posX1 + 1;
			posY1 = 180 - (fft_outputbuf[i+380] * (400.0f / 4096.0f));
			posY2 = 180 - (fft_outputbuf[i+381] * (400.0f / 4096.0f));
			printf("line %d,%d,%d,%d,%d\xff\xff\xff", posX1, posY1, posX2, posY2, 31);
		}
		for(i=0;i<263;i++)
		{
			posX1 = i;
			posX2 = posX1 + 1;
			posY1 = 270 - (fft_outputbuf[i+760] * (400.0f / 4096.0f));
			posY2 = 270 - (fft_outputbuf[i+761] * (400.0f / 4096.0f));
			printf("line %d,%d,%d,%d,%d\xff\xff\xff", posX1, posY1, posX2, posY2, 31);
		}		
		arm_max_f32(fft_outputbuf + 1 ,1023,&max,&max_idex);
		printf("page1.t3.txt=\"%.2lf\"\xff\xff\xff",max);
		printf("page1.t4.txt=\"%d\"\xff\xff\xff",max_idex);	
	}
